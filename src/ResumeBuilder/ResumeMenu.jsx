...
import { Menu, MenuButton, MenuList, MenuItem, Button, Spacer, Badge, Heading, Text, useBreakpointValue } from '@chakra-ui/react'
import { ChevronDownIcon } from '@chakra-ui/icons'


function ResumeMenu({ resumeList, setResume, setNewResumeMode, resume }) {
    const menuWidth = useBreakpointValue({ base: '100%', md: '300px' })
    const textSizes = useBreakpointValue({ base: 'sm', md: 'md' })
    if (resumeList.data.length === 0) return <>
        No resumes found
    </>
    return <>
        <Menu>
            ...
            <MenuList w={menuWidth}>
                {resumeList.data.map((resume) => (
                    ...
                    >{resume.identifier}</Heading>
                        <Spacer />
                        {//check if first resume in list
                            resumeList.data[0]._id === resume._id &&
                            <Badge ml="1" colorScheme="blue" size={'sm'} variant="outline">
